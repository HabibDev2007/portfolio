  ...
  return (
    <section className="section-pad relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-4xl px-6 text-center">
        <Reveal>
          <p className="mono-label mb-4 text-sm text-signal">// 08 — what's next</p>
          ...
          {goals.map((goal, i) => (
            <Reveal key={i} delay={i * 0.05}>
              <div className="flex items-center gap-3 rounded-xl border border-line/10 bg-surface/40 px-5 py-4">
                <ArrowUpRight size={16} className="shrink-0 text-signal" />
                <p className="text-sm text-ink sm:text-base">{goal}</p>
              </div>
            </Reveal>
          ))}
        </div>
